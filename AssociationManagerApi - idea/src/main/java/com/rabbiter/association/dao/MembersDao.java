...
public interface MembersDao extends BaseMapper<Members> {
    ...
    @Select("SELECT " +
            "m.team_id teamId, t.name teamName, u.id userId, u.name userName, u.user_name accountName, u.phone userPhone " +
            "FROM members m " +
            "JOIN users u ON m.user_id = u.id " +
            "JOIN teams t ON m.team_id = t.id " +
            "WHERE m.team_id = #{teamId} " +
            "ORDER BY u.name ASC")
    List<Map<String, Object>> qryMemberOptionsByTeamId(@Param("teamId") String teamId);
}
